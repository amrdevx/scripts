...
screen=$(xrandr | awk '/ connected/ {print $1}' | dmenu)
fps=$(xrandr | awk '/*.*/ {print $2}' | tr -d '*+' | dmenu)
...
if [ $(pidof ffmpeg) ]; then
	...
	ffmpeg -video_size $resolution -framerate $fps -f x11grab -i :0.1 "$output/$(date '+%s').mkv"
#fi
#slstatus &
